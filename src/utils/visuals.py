...
def visualize_regression_results(
    ...
    figures = {}
    figures["scatter"] = _create_scatter_plot(
        y_true, y_pred, labels, titles, fig_size, save_path, show_figures, fit_degree
    )
    figures["residuals"] = _create_residuals_plot(
        y_pred, residuals, labels, titles, fig_size, save_path, show_figures
    )
    figures["distribution"] = _create_distribution_plot(
        residuals, labels, titles, fig_size, save_path, show_figures
    )
    figures["qq_plot"] = _create_qq_plot(
        residuals, titles, fig_size, save_path, show_figures
    )
...
def _create_scatter_plot(
    ...
    if fit_degree > 1:
        ...
        plt.plot(
            x_curve, y_curve, 'r-', label=f"Ajuste polinómico (grado={fit_degree})"
        )
    ...
    return _save_and_show_figure(
        fig_scatter, save_path, "scatter_plot.png", show_figures
    )


def _create_residuals_plot(
    y_pred, residuals, labels, titles, fig_size, save_path, show_figures
):
    """
    ...
    plt.axhline(
        y=0, color='r', linestyle='--', label="Residuales nulos"
    ...
    return _save_and_show_figure(
        fig_residuals, save_path, "residuals_plot.png", show_figures
    )
...
def _create_distribution_plot(
    ...
    return _save_and_show_figure(
        fig_dist, save_path, "residuals_distribution.png", show_figures
    )
...
def _create_qq_plot(residuals, titles, fig_size, save_path, show_figures):
    ...
    def erfinv(y):
        ...
        return np.sign(y) * np.sqrt(
            np.sqrt(term1**2 - ln / a) - term1
...
def _setup_plot_with_optimal_value(
    lambdas,
    metric_scores,
    metric_name,
    is_minimize=None,
    ...
    # plot the line
    sns.lineplot(
        x=lambdas, y=metric_scores, ax=ax, color='royalblue', label=metric_name
    )

    # determine if we're minimizing or maximizing
    if is_minimize is None:
        is_minimize = any(
            term in metric_name.lower() for term in ['error', 'loss', 'mse', 'mae']
        )
...
def plot_weights_vs_lambda(lambdas, weights, feature_names, custom_titles=None):
    ...
    data_long = pd.melt(
        data, id_vars=['lambda'], var_name='feature', value_name='weight'
    )
...
def plot_performance_metrics(lambdas, mse_scores, r2_scores, random_state=None):
    ...
    min_lambda, min_mse = plot_metric_vs_lambda(
        ax1, lambdas, mse_scores, metric_name="MSE", marker=None
    )
    max_lambda, max_r2 = plot_metric_vs_lambda(
        ax2, lambdas, r2_scores, metric_name="R2", marker=None
    )
...
def plot_cv_results(
    ...
    if isinstance(cv_metrics_scores, dict):
        ...
        if metric_name not in cv_metrics_scores:
            ...
            raise ValueError(
                f"The metric '{metric_name}' is not available. Available metrics: {available_metrics}"
            )
