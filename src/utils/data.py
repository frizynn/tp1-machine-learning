...
def get_nan_features(df: pd.DataFrame):
    ...
    nan_features = nan_features[
        nan_features > 0
    ]  # filters only columns with at least one NaN
...
def split_test_train(
    ...
    if normalize:
        ...
        train_std = train_std.replace(
            0, 1
        )  # avoids division by zero in constant features
...
def load_and_prepare_data(
    target_column, df=None, data_path=None, feature_columns=None, transform_target=None
):
    """
...
def _handle_feature_engineering(
    ...
    for op in operations:
        ...
        new_features[feature_name] = (
            operation(df) if callable(operation) else df.eval(operation)
        )
...
def process_dataset(
    ...
    new_features_df, feature_stats = _handle_feature_engineering(
        df, feature_engineering_ops
    )
        ...
        for feature in zone_interaction_features:
            if feature in df.columns:
                for zone in range(kmeans_model.n_clusters):
                    ...
                    zone_interactions[interaction_name] = df[feature] * (
                        df['location_zone'] == zone
                    )
...
def compare_feature_impact(model_results_dict, property_dfs, feature_name='has_pool'):
    ...
    if not isinstance(model_results_dict, dict) or (
        isinstance(model_results_dict, dict) and 'model' in model_results_dict
    ):
    ...
    for model_name, model_results in model_results_dict.items():
        ...
        coef_dict = None
        if hasattr(model_results["model"], 'get_coef_dict') and callable(
            model_results["model"].get_coef_dict
        ):
            coef_dict = model_results["model"].get_coef_dict()
        elif (
            hasattr(model_results["model"], 'coef_dict')
            and model_results["model"].coef_dict
        ):
            coef_dict = model_results["model"].coef_dict

        if not coef_dict or feature_name not in coef_dict:
            print(
                f"Warning: Feature '{feature_name}' not found in model '{model_name}'. Skipping."
            )
            continue
...
def cross_validate_lambda(
    ...
    if transform_target and inv_transform_pred is None:
        raise ValueError(
            "transform_target and inv_transform_pred cannot both be None. If transform_target is used, inv_transform_pred must be used to invert the transformation to calculate metrics."
        )
    ...
    for lambda_val in lambdas:
        ...
        for train_idx, val_idx in folds:
            ...
            score = evaluate_model(
                model, X_val, y_val, metrics, inv_transform_pred, y_pred_test
            )
    ...
    cv_metrics_scores = {
        key: np.array(scores) for key, scores in cv_metrics_scores.items()
    }
    ...
    min_cv_metrics = {
        key: cv_metrics_scores[key][optimal_idx[key]] for key in cv_metrics_scores
    }
